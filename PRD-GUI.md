...
- **FR-G2.10 (P1)** **Live connection-string field** in the connection modal: the URL field is prefilled from the stored connection when editing and **continuously rebuilt as the parameter fields change** (engine, host, port, user, password, database, SSL mode) — never blank when parameters exist. The password inside the URL is masked unless the reveal-password toggle (FR-G2.9) is on. Pasting a URL and clicking Import still populates the fields (a masked `•••` password in a pasted URL never overwrites the real one), after which the field re-normalizes to the canonical constructed string. *(E2/S2.9)*
